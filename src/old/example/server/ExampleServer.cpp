...
std::unique_ptr<RocketRoutingHandler> createHTTP2RoutingHandler(const std::shared_ptr<ThriftServer>& server) {
  return std::make_unique<RocketRoutingHandler>(*server);
}

template <typename ServiceHandler> std::shared_ptr<ThriftServer> newServer(int32_t port) {
